$(document).ready(function () {
        ...
        $.each(costsTables, function (key, value){
            ...
            if (init_price.constructor == Object) {
                ...
                if (strikethrough.length < 1) {
                    duration.find('[data-length="' + length +'"] .price-per-length').prepend('<s class="strikethrough"><span class="vat" data-price="0.0">0.0</span> β¬</s>');
                }
        ...
        function update_summary_option_price () {
            $('.configurator input:checked:not(.remove_spec)').each(function () {
                ...
                if(vat.get_price() > 0){
                    ...
                    vatToUpdate.find('.vat').text(vatToUpdate.find('.vat').text() + ' β¬');
                }
            });
        }
     ...
     */
        ...
        function source_included_item (vat, target_spec) {
            ...
            if (target_spec.text().match('β¬') == null) {
                target_spec.find('.vat').after(' β¬');
            }
        }
